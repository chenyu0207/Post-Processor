...
class mysqldb {
    ...
    public function __construct(array $conf) {
        ...
        if (!in_array($conf['name'], $this->_dbNameArr)) {
            $arr = $this->_dbNameArr;
            $arr[] = $conf['name'];
            $this->_dbNameArr = $arr;
            $this->_connect();
    ...
    protected function _connect() {
        ...
        $options = $this->_pconnect ? array(\PDO::ATTR_PERSISTENT=>true, \PDO::MYSQL_ATTR_USE_BUFFERED_QUERY=>true) : array();
        ...
        // var_dump(\PDO::MYSQL_ATTR_USE_BUFFERED_QUERY);
        // die;
    ...
    protected function _doQuery($sql='') {
        // $prefixFun = debug_backtrace();
        // print_r($prefixFun);
        // die;
        // $log = " file:".$prefixFun[0]['file'];
        // $log .= " class:".$prefixFun[0]['class'];
        // $log .= " function:".$prefixFun[0]['function'];
        // $log .= " args:".json_encode($prefixFun[0]['args']);
        // $log .= " line:".$prefixFun[0]['line'];
        // $log .= " \n SQL:$sql";
        ...
        $this->_sql = $sql;
// file_put_contents('./sql.log', 'doQuery: '.$sql."\n", FILE_APPEND);
        if (empty($this->_sql)) {
            $error = "sql is null! ";
            throw new \Exception($error);
        }
...
// file_put_contents('./sql.log', 'doQuery: server has gone away'."\n", FILE_APPEND);
                sleep(1);
                $this->_connect();
    ...
    protected function _doExec($sql='') {
        $this->_sql = $sql;
// file_put_contents('./sql.log', 'doExec: '.$sql."\n", FILE_APPEND);
        try {
...
// file_put_contents('./sql.log', 'doExec: server has gone away'."\n", FILE_APPEND);
                sleep(1);
                $this->_connect();
                return self::$_dbh->exec($this->_sql);
            }
